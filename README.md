...
2. Запустите приложение в режиме разработки, выполнив команду `npm run dev`.
3. Перейдите на страницу `http://localhost:3000` в вашем браузере, чтобы увидеть работу приложения.
...
10. **prizes.js** - вторая страница приложения, предлназначанная для отображения списка призов, полученных пользователем.

    Принимаемые Props:
    - `address`  - адрес Ethereum кошелька пользователя
    - `isConnected`  - флаг, указывающий, подключен ли пользователь к сети Ethereum

    Дочерние компоненты
    - `Meta` - компонент для установки мета-тегов страницы
    - `Header` - компонент, содержащий заголовок страницы
    - `Connector` - компонент для подключения пользователя к сети Ethereum
    - `PrizesForm` - компонент, содержащий список призов пользователя
    - `Footer` - компонент, содержащий подвал страницы
  
     Возвращает разметку для страницы с заголовком, компонентом подключения, списком призов пользователя и подвалом страницы.

11. **PrizesForm.js** - компонент предназначен для отображения списка призов, которые пользователь получил за отправку сообщений в контракт. Компонент получает два пропса: `address` - адрес кошелька пользователя на Ethereum, и `isConnected` - флаг, указывающий, подключен ли пользователь к сети Ethereum.
